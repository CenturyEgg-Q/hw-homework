...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    "*** YOUR CODE HERE ***"
    if(num_rolls == 0):
       temp = free_bacon(opponent_score)
...
def swine_align(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4a
    "*** YOUR CODE HERE ***"
    i,j = player_score,opponent_score
    if (i <= 0) or (j <= 0):
        return False
    while j != 0:
        t = i % j
    ...
    while True:
        if who == 0:
            ...
            if not extra_turn(score0,score1):
                who = other(who)
        if who == 1:
            num0 = strategy1(score1,score0)
            score1 += take_turn(num0,score0,dice)
            say = say(score0,score1)
            if score0 >= goal or score1 >= goal:
                break
...
def make_averaged(original_function, trials_count=1000):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    def averaged(*args,trials_count= trials_count):
        sums = 0
        for i in range(trials_count):
            temp = original_function(*args)
            sums += temp
        return float(sums)/trials_count
    return averaged
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    num = 10
    for i in range(1,11):
        averaged_dice = make_averaged(roll_dice,trials_count = trials_count)
        temps = averaged_dice(i,dice)
        if temps >= sums:
            if temps == sums:
                continue
            sums = temps
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    num_rolls = bacon_strategy(score,opponent_score,cutoff = cutoff,num_rolls = num_rolls)
    if num_rolls == 0:
        return 0
    temp = free_bacon(opponent_score)
    score1 = score + temp
    if extra_turn(score1,opponent_score):
        return 0
    return num_rolls
    # Replace this statement
    # END PROBLEM 11
